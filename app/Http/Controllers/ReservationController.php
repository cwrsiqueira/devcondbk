...
class ReservationController extends Controller
{
            ...
            if($area)
            {
                ...
                if($can)
                {
                    ...
                    foreach($times as $time)
                    {
                        $timeList[] = [
                            'id' => date('H:i:s', $time),
                            'title' => date('H:i:s', $time).' - '.date('H:i', strtotime('+1 hour', $time)),
                        ];
                    }
    ...
    public function setReservation($id, Request $request)
    {
        ...
        if(!$validator->fails())
        {
            ...
            if($unit && $area)
            {
                ...
                // Veriricar se não existe outra reserva no mesmo dia/hora
    ...
    public function delMyReservation($id)
    {
        ...
        if($reservation)
        {
            ...
            {
                $array['error'] = 'Exclusão da reserva não permitida';
                return $array;
            }
        }
        else
